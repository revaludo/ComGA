...
if __name__ == "__main__":
    # Usage
    data = Preprocessor()
    data.load_data("/data/ldd/data/ml-latest-small/ratings.csv", sep=",", columns="UIRT")
    data.drop_duplicates()
    data.filter_data(user_min=1, item_min=1)
    data.remap_data_id()
    # data.split_data_by_leave_out(valid=1, test=1)
    data.split_data_by_ratio(train=0.8, valid=0.0, test=0.2, by_time=True)
    data.save_data("/data/ldd/data/ml-latest-small/")
